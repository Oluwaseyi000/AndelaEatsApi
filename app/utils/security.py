                    ...
                    for rule in rules:
                        ...
                        for validator in validators:
                            ...
                            if validator == 'int' and type(payload[request_key]) is str and not payload[
                                    request_key].isdigit():
                                error.append(
                                    {request_key: ['Bad Request - {} must be integer'.format(request_key)]})
                            ...
                            if validator == 'float':
                                try:
                                    ...
                                    error.append(
                                        {request_key: ['Bad Request - {} must be float'.format(request_key)]})

                            if (validator == 'required' and request_key not in payload) or payload[request_key] == '':
                                error.append(
                                    {request_key: ['Bad Request - {} is required'.format(request_key)]})

                            if validator.find('max') > -1:
                                max_value = int(validator.split('-')[1])
                                if int(payload[request_key]) > max_value:
                                    error.append({request_key: ['Bad Request - {} can only have a max value of {}'.format(
                                        request_key, max_value)]})

                            if validator.find('min') > -1:
                                min_value = int(validator.split('-')[1])
                                if int(payload[request_key]) < min_value:
                                    error.append({request_key: ['Bad Request - {} can only have a min value of {}'.format(
                                        request_key, max_value)]})

                            if validator.find('length') > -1:
                                length_value = int(validator.split('-')[1])
                                if len(str(payload[request_key])) > length_value:
                                    error.append({request_key: ['Bad Request - {} can only have a len of {}'.format(
                                        request_key, max_value)]})
                            ...
                            if validator == 'exists':
                                ...
                                if type(payload[request_key]) == int:
                                    ...
                                    if not v:
                                        error.append({request_key: ['Bad Request - {} contains invalid {}(s) for {} table '
                                                                    .format(request_key, column_name,
                                                                            repo_name)]})
                            ...
                            if validator == 'date' or validator == 'time':
                                ...
                                try:
                                    ...
                                    error.append({request_key: ['Bad Request - {} should be valid {}. Format: {}'
                                                                .format(request_key, formatter.get('type'), formatter.get('format'))]})

                            if validator == 'list' and type(payload[request_key]) is not list:
                                error.append(
                                    {request_key: ['Bad Request - {} must be a list'.format(request_key)]})
            ...
            def decorated(*args, **kwargs):
                ...
                for name, val in controller.get_params_dict().items():
                    if name.endswith('ted_at'):
                        try:
                            kwargs.__setitem__(name, datetime.strptime(
                                kwargs.get(name), '%Y-%m-%d'))
                        except Exception:
    ...
    def validate_enums(validator, key, value):
        ...
        if split_validator[0] == 'enum':
            ...
            if value not in enum_values:
                ...
                return {'msg': "Bad Request - '{}' is not a valid value for key '{}'.values must be any of the following {}".format(value, key, enum_values)}
    ...
    def validate_email(cls, validator, value):
        ...
        if validator == 'email':
            if not cls.EMAIL_REGEX.match(value):
                return {'msg': "Bad Request - '{}' is not a valid email address.".format(value)}
    ...
    def validate_url(cls, validator, value):
        ...
        if validator == 'url':
            if not re.match(cls.URL_REGEX, value):
                return {'msg': "Bad Request - '{}' is not a valid url.".format(value)}
